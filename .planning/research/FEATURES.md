...
| DF-10 | **Progressive Scaling Roadmap** — artifacts produced at Startup weight with documented upgrade paths to Growth and Enterprise maturity levels (aligned to SDLC-v2) | Product/Strategy Lead | Medium | None | Built-in maturity model means the ISMS/AIMS grows with the company without needing to restart |
...
| AF-08 | **Full SDLC-v2 Governance Implementation** — 5 boards, dual ARTs, all 18 sections at Enterprise weight | Explicitly out of scope per PROJECT.md; Startup overlay is the plan | Growth phase as documented in SDLC-v2 | "Drown in governance friction before ARR justifies it" (CTO/CISO assessment) |
